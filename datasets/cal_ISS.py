...
class Kitti_cal_overlap(object):
    ...
    def __init__(self, root, pose_root, savepath, split, downsample):
        ...
        for scene in self.scene_list:
            ...
            # self.poses[scene] = np.array([odometry_to_positions(odo) for odo in self.poses[scene]])
    ...
    def make_poses_pair(self, drive):
        ...
        while curr_time in inames:
          next_time = np.where(valid_pairs[curr_time][curr_time:curr_time + 200])[0]
          if len(next_time) == 0:
            curr_time += 1
          else:
          ...
          if next_time in inames:
            ...
            curr_time = (next_time + 1) - ((next_time - curr_time)//2)
            mid_ids_list = []
    ...
    def load_ply(self, fname0, fname1, fname_mid, downsample, aligned=True): #join(data_dir, f'{ind}.bin')
        ...
        pcd3.points = o3d.utility.Vector3dVector(xyz1)
        ...
        for a in fname_mid:
            ...
            pcd_mid.points = o3d.utility.Vector3dVector(xyz_mid)
        ...
        if aligned is True:
            for pcd_mid in first_half:
                reg = o3d.pipelines.registration.registration_icp(
                    ...
                    o3d.pipelines.registration.ICPConvergenceCriteria(relative_fitness=1e-12, relative_rmse=1e-12, max_iteration=100000))
            ...
            if np.linalg.norm(T_accumulated[:3, 3]) < self.MIN_DIST / 2:
                return np.array(pcd0.points), np.array(pcd1.points), False, T_accumulated
    ...
    def load_all_ply(self, downsample):
        # pts_filename = join(self.savepath, f'kitti_{self.split}_{downsample:.3f}_points_na.pkl')
    ...
    def cal_overlap(self, downsample):
        # overlap_filename = join(self.savepath, f'kitti_{self.split}_{downsample:.3f}_overlap_na.pkl')
        # keypts_filename = join(self.savepath, f'kitti_{self.split}_{downsample:.3f}_keypts_na.pkl')
        ...
        for i, key_idx in enumerate(self.pair_keys):
            ...
            cloud = pcl.PointCloud(anc_pts)
            iss = cloud.make_iss_keypoint()
            iss.setSalientRadius(6 * self.downsample)
            iss.setNonMaxRadius(4 * self.downsample)
            iss.setMinNeighbors(5)
            iss.setThreshold21(0.975)
            iss.setThreshold32(0.975)
            keypoints = iss.compute()
            ...
            # 클라우드 추가
            viewer.ShowMonochromeCloud(cloud)  # 클라우드 시각화

            # 키포인트 추가
            viewer.SelfRender(keypoints.make_Object())
