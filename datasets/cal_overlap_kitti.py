...
class Kitti_cal_overlap(object):
    ...
    def load_ply(self, fname0, fname1, fname_mid, downsample, aligned=True): #join(data_dir, f'{ind}.bin')
        ...
        pcd0_org = open3d.geometry.PointCloud()
        pcd1 = open3d.geometry.PointCloud()
        pcd0.points = open3d.utility.Vector3dVector(xyz0)
        pcd0_org.points = open3d.utility.Vector3dVector(xyz0)
        ...
        pcd0_org = pcd0_org.voxel_down_sample(voxel_size=downsample)
        ...
        if aligned is True:
            ...
            pcd0, _ = pcd0.remove_statistical_outlier(nb_neighbors=20, std_ratio=1.0)
            pcd1, _ = pcd1.remove_statistical_outlier(nb_neighbors=20, std_ratio=1.0)
            ...
            if np.linalg.norm(T_accumulated[:3, 3]) < 2.0:
                self.T.append(T_accumulated)
                return np.array(pcd0.points), np.array(pcd1.points), False

        return np.array(pcd0.points), np.array(pcd1.points), True
